network.sim <-
  function(n = 1500, # Total # of time/new individuals: n = 1500                                                #
    ...
    for (t in 1:size1){
      ...
      ### The first group for multinomial is not group to any of existing groups
      if(j != 1){
        # Parameter Updates:
        x1_t.plus.1 = x1_jt
    ...
    ### we want more samples when there are more groups
    n_clus = c(n_clus, 1)
    for (t in (size2+1):n){
      ind = 10
      if(t == (size2+1)){
        x1_new1 = round(rnorm(1, meann[ind], sdd),0)
        r2_num_jt_new = propp[10]*20
        r2_denom_jt_new = 20
        x1_jt = c(x1_jt, x1_new1)
        r2_num_jt = c(r2_num_jt, r2_num_jt_new)
        r2_denom_jt = c(r2_denom_jt, r2_denom_jt_new)
    ...
    return(
      list(
        #s_jt = s_jt,
        # Cluster size at time t
        k_0 = k_0,
        # Total # of clusters at baseline
        #k_t = k_t,
        # Total # of clusters at time t
        #j.vec = j.vec,
        x1_jt_minus_x_new = x1_jt_minus_x_new,
        x2_jt_minus_x_new = x2_jt_minus_x_new,
        # dim: [n-(k_t-k_0)] * (k_t) or (k_t-1)
        Z_vec = Z_vec,
        x_prop = x_prop,
        x_his = x_his
      )
